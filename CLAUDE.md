...
*Last updated: 2026-04-30 — Artifacts Phase 5 added (operating-statement guard + universal tabular formatting standard). 7-pass live-iteration session shipped on `feature/unified-ui` between commits `c2d18dd` and `f21f2bf`. Net: hard programmatic enforcement of OS structural rules (single table, 3-col canonical shape, no property metadata, subtype declaration) + reusable renderer-level formatting baseline (accounting numbers, bold subtotals, indent depths, section-divider header merge, numeric-only rules) that applies to every tabular artifact going forward. Cowork-side memory `feedback_tabular_artifact_formatting.md` codifies the standard. BASE_INSTRUCTIONS T-12 strict content rules (~80 lines) now superseded by guard but not yet removed. Branch state: `feature/unified-ui` active, merge to main pending.*
