...
**Studio shell (new, additive) — `/studio/[projectId]` route.** A third, isolated project surface (`LSCMD-UIWRAP-LEFTNAV-0623-JB1`) that relocates the folder/sub-tab tabs into a collapsible **two-level left rail** (generated from `createFolderConfig`), with reused `CenterChatPanel` in the center and `ProjectContentRouter` output inside a reused `RightContentPanel` on the right. Top-level route (escapes the `/w/` `WrapperLayout`); composes its own `WrapperUIProvider` + project providers. Components in `src/components/studio/` (`StudioShell.tsx`, `StudioSidebar.tsx`), styles in `src/styles/studio.css`. Does not alter classic `/projects/[id]` or `/w/`. Chat-driven navigation: `navigate_to_screen` tool lets Landscaper open any folder screen in the studio right panel (e.g., "show me the budget" → budget grid inline). Studio subscriber in `landscape-command-bus.ts` bridges tool output → panel. Generalized no-fabrication guard (Jun 24) prevents Landscaper from inventing financial figures — all project financials (IRR, NPV, equity, cash flow, etc.) must come from `calculate_*` tools, not model reasoning.
