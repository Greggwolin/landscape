...
|------|---------|---------|----------|
| `feedback-log-near-push` | Every 2 minutes | Catch state changes (new feedback, status flips) within ~2 min of the write. | Compares `MAX(GREATEST(created_at, started_at, addressed_at, closed_at))` to the artifact's current `generated_at`. If DB is newer → run the same refresh routine the nightly skill uses. If not → exit silently. Cheap when nothing has changed (single SELECT). |
