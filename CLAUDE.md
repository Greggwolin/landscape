...
*Last updated: 2026-04-30 (chat hx) — F-12 Proforma server-derived from T-12 (`get_proforma` tool). Live test on Chadron Terrace 2026-04-30 produced an LLM-composed F-12 with 5 composition bugs (collapsed expense detail, phantom Pest Control line, wrong unit-mix bucket counts, blank commercial line, no composition-fidelity guard). Fix: new `get_proforma` tool composes the F-12 server-side from the same operations endpoint that powers `get_operating_statement` + project growth rates pulled from `core_fin_growth_rate_sets` (default 3% / 3% if unset). Vacancy / credit / concessions / management-fee percentages preserved (values recomputed from grown GPR / EGI). Replacement reserves per-unit grows by expense rate. Single-table schema mirrors T-12 exactly — same line items, same row order, no unit-mix breakdown in income. Phase 5 OS guard's structural rules still validate the output. Tool added to `INCOME_PROPERTY_TOOLS` in `tool_registry.py`. BASE_INSTRUCTIONS section "F-12 PROFORMA — USE get_proforma, DO NOT COMPOSE" routes the model. `artifact_subtype: f12_proforma` description points at the new tool. Files: `backend/apps/landscaper/tools/proforma_tools.py` (new), `tool_executor.py` (import added), `tool_registry.py`, `tool_schemas.py`, `ai_handler.py`. Tests: `backend/apps/landscaper/tools/tests/test_proforma_tools.py` (10 cases). Verify-and-ship handoff: `Landscape app/F12_VERIFY_AND_SHIP.md`. Carryover: rent-roll guard queued (same composition-drift class — observed top of chat hx); `current_proforma` server-derived path not yet built. Prior session-log entry: Artifacts Phase 5 (operating-statement guard + universal tabular formatting standard) shipped on `feature/unified-ui` between commits `c2d18dd` and `f21f2bf`. BASE_INSTRUCTIONS T-12 strict content rules (~80 lines) still superseded but not yet removed. Branch state: `feature/unified-ui` active, merge to main pending.*
*Last audit: 2026-04-30 — Alpha Readiness Assessment (15-step workflow audit, includes Artifacts system)*
*Landscaper tool count: **~269 registered** (+1 from chat hx: `get_proforma`; +5 artifact tools and `get_operating_statement` added Apr 25–30; 3 LoopNet tools registered but not advertised — gx14 deferral). New since prior audit: `create_artifact`, `update_artifact`, `get_artifact_history`, `restore_artifact_state`, `find_dependent_artifacts`, `get_operating_statement`, `get_proforma`. Excel audit phases implemented: 0, 1, 2, 2f, 3, 4, 6, 7-partial. Phase 5 (Python waterfall replication) is the only remaining major piece.*
