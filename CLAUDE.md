...
**Lingering `tbl_container` references — RESOLVED 2026-06-08:** The Nov-2025 rename left ~60 additional callsites. The six most user-facing were fixed 2026-05-16; the rest were completed on 2026-06-08:
- ~~**`backend/apps/containers/models.py`**~~ — ✅ RESOLVED (#42). `Container.Meta.db_table` now points to `tbl_division`. Python field names preserved (`container_id`, `container_level`, etc.) with `db_column` remaps to the real columns (`division_id`, `tier`, `division_code`, `parent_division_id`). API contract unchanged.
- ~~**6 report generators**~~ — ✅ RESOLVED (#32, #40, #41). All repointed from `tbl_container` to `tbl_division`.
- ~~**15+ Next.js API routes**~~ — ✅ RESOLVED (#42). 12 legacy routes migrated to new column names with `SELECT ... AS` aliases preserving API output.
- ~~**Dead models**~~ — ✅ RESOLVED (#46). `ContainerType` and `ContainerCostMetadata` models + API surface deleted (-244 lines).
- ~~`src/types/database.ts`~~ — ✅ RESOLVED (#47). Regenerated with circular-namespace bug fix.
- **Remaining:** `management_overhead.container_id` is the real DB column name (not renamed to `division_id`). Left intentionally — it's a different table with its own migration history.
...
2. **A heading wraps only where wrapping earns its place, and the column's ALIGNMENT decides** (Gregg, 2026-09-17 — replaces the flat "multi-word headers wrap"). A LEFT-aligned column holds words: it is the stub column, already the widest in the table, and its own data forces it wider than its heading — so its heading stays on ONE LINE ("Line / Item" stacked is two headings and costs the row its reading line). A RIGHT or CENTER aligned column holds figures: it is narrow by nature and its heading is routinely the widest thing in it, so it MAY wrap rather than widen the column past its own data. Implemented in the two artifact renderers (`ArtifactRenderer.module.css`, `ScheduleArtifact.module.css`) and, on Gregg's instruction of 2026-09-17, in the AG-Grid surfaces (`src/app/rent-roll/rent-roll-grid.css` — `.ag-right-aligned-header`, which `type: 'numericColumn'` supplies, is what separates the two cases) and the classic rent-roll table (`src/styles/property-page.css`). Plain CoreUI tables that set no `white-space` on `th` inherit the browser default and already keep a heading on one line until the column is squeezed; state the rule explicitly there when one is next touched.
...
2b. **Every artifact carries its own print control** (2026-09-17). The block renderer and the schedule renderer each print the artifact ALONE — written into a hidden frame, never the page around it — on an explicitly light sheet regardless of the dark working surface, with the heading row repeating across page breaks and rows never split. The schedule renderer prints WHAT IS ON SCREEN (the rows the filter left, the columns the rung and chips left) and says so in a footnote. Shared helper: `src/components/wrapper/printArtifact.ts`.
...
| 7 | Landscaper Chat | ✅ WORKS | 282 tools (master list refreshed 2026-07-24; +5 deterministic schedule artifact tools, +`open_clarification` tool, +`control_map_overlay` SS16 chat-driven site-plan drape control, demo question battery runner, fabrication guard figure-trace instrumentation); thread-based with unassigned (pre-project) threads, tightened firing discipline, cross-property fabrication blocked |
...
- Reports system complete: 20 generators with real SQL + PDF/Excel export; data_readiness flags updated. Cash-flow reports (RPT_12/17/18/19) share `proforma_base.py` unified renderer (Jun 2026)
...
**Legacy (folder/tab) — `/projects/[id]` route.** The original ARGUS-style 8-folder layout with two-row tabs and `?folder={folder}&tab={tab}` URL state. `ProjectContentRouter.tsx` still maps folder/tab combos to legacy components, and `FolderTabs.tsx` and `useFolderNavigation.ts` are still wired. `src/lib/utils/folderTabConfig.ts` **is live** — it exports `createFolderConfig(...)` (the property-type/analysis-type-aware folder+sub-tab tree) and is consumed by `useFolderNavigation.ts`. (A prior note here claimed this file "no longer exists"; that was wrong — corrected `LSCMD-UIWRAP-LEFTNAV-0623-JB1`.) Post-login now redirects to `/w/dashboard` (as of `270c72e6`, May 15 2026). Legacy routes (`/`, `/register`) also redirect to `/w/` counterparts via `next.config.ts`.

**Studio shell (deterministic fallback) — `/studio/[projectId]` route.** Was briefly THE project shell (`741fb7fc`, Jun 25 — `/w/projects/[projectId]` routes funneled into it), but the funnel was **reverted Jul 19 2026** (PR #181, `ba0c0c83`, session RF33-CHATSURFACE-0719) per product direction: **the `/w/` chat surface with the right artifacts panel is the working project surface.** `/w/projects/[projectId]` now mounts the full pre-funnel provider stack (project fetch, ProjectContextShell, WrapperChatProvider, ModalRegistryProvider with 17 registered designed forms via side-effect import of `wrapper/modals`, WrapperProjectProvider, LandscapeCommandSubscriber). Where a designed form exists, Landscaper opens the existing component via `open_input_modal` → ModalRegistry; artifacts are for generated content only. Studio remains fully functional by direct URL as the quiet deterministic fallback — de-emphasized, not deleted. Same PR restored the `/w/` reports + map page bodies (gutted to null stubs by #165 while unreachable) and fixed a pre-existing thread-selection bug (`get_or_create_active_thread` now excludes archived threads). Three-panel layout: collapsible **two-level left rail** (generated from `createFolderConfig`), reused `CenterChatPanel` in the center, `ProjectContentRouter` output inside a reused `RightContentPanel` on the right. Top-level route (escapes the `/w/` `WrapperLayout`); composes its own `WrapperUIProvider` + project providers. Components in `src/components/studio/` (`StudioShell.tsx`, `StudioSidebar.tsx`), styles in `src/styles/studio.css`. Chat-driven navigation: `navigate_to_screen` tool lets Landscaper open any folder screen in the studio right panel (e.g., "show me the budget" → budget grid inline). Studio subscriber in `landscape-command-bus.ts` bridges tool output → panel. Two-layer no-fabrication guard (Jun 24–26): (1) reply-assembly guard in `ai_handler.py` catches fabricated financial text in responses, (2) `financial_artifact_guard.py` (`d28d358c`, Jun 26) hard-prevents fabricated financial cards at `create_artifact` time — rejects artifacts stating money/percent/multiple figures when no numbers-producing tool ran during the turn. Both share detection regexes (lazy import to avoid drift). Prompt rules (`c2f7e863`) enforce three-way routing: whole-screen asks → navigate; cards with tool data → render; no tool available → refuse+navigate.
...
**OCR seam (Phase 1, 2026-06-20 — LSCMD-PLANEXTRACT-P1-0620-ot4):** The integration point now exists in `auto_classifier.extract_text_from_bytes()` → `_extract_pdf_with_ocr()`. When a PDF yields no native text layer it routes to this seam, which is **flag-gated** (`settings.ENABLE_OCR`): with the flag off (or OCRmyPDF/Tesseract/Ghostscript unprovisioned) it returns an **explicit error** rather than a silent empty; with the flag on and OCRmyPDF installed it adds a text layer and re-extracts. Provisioning the binaries + enabling the flag is the remaining work to close the scanned-PDF gap.

### Site-Plan Image Extraction (Phase 1 — extract + place)

**2026-06-20 (LSCMD-PLANEXTRACT-P1-0620-ot4):** `extract_plan_image` Landscaper tool renders a plan/plat/site-plan PDF page (or a clipped region) to a transparent-capable RGBA PNG via `MediaExtractionService.render_plan_crop()` (`_render_page_png` shared helper; `page.get_pixmap(clip=..., alpha=True)`), saved to Django `default_storage`. The tool is two-step (preview with `doc_id` only → confirm with `page` + `crop_bbox`) and never writes an overlay itself. The frontend (`MapTab.applyExtractedPlanOverlay`, fired via a `landscaper:place_plan_overlay` CustomEvent) re-uploads the PNG through the existing `useUploadThing` overlay flow and drapes it via the existing overlay editor; on Save, `createOverlay` persists source-document provenance — new nullable columns `source_doc_id` / `source_page` / `source_crop_bbox` on `landscape.tbl_project_overlay` (migration `20260620_overlay_source_provenance`). Out of scope (later phases): auto boundary detection + irregular-mask transparency, vector tracing, the `project_map_features` role store, jurisdiction discovery.

**2026-06-20 (LSCMD-PLANEXTRACT-CP-0620-ot4) — click-to-extract canvas (D15) + control-point georeferencing (D16). Supersedes the auto-detect approach to extraction.** Uploading a site plan now opens an interactive trace canvas (`src/components/map-tab/extract/PlanExtractCanvas.tsx`) fed a CORS-clean data URL; the user outlines region(s) and exports a transparent PNG per region (`onExportRegion` → `MapTab.handleExportRegion` re-uploads via `useUploadThing` and drapes). Placement is georeferenced from user-placed **control points** (`src/lib/gis/controlPoints.ts`: `georeference()` → 2 pts similarity / 3 affine / 4+ projective, fit in a local metric frame; `snapToVertex()`; `recommendTpsWarp()`): the user clicks a point on the extracted-image thumbnail (image px) then the matching parcel vertex on the live map — `map.queryRenderedFeatures` on the parcel layers → `snapToVertex` (8 m) → `overlayEditor.setCorners()` previews the transform live (kind + RMS shown). Save persists the inputs in new nullable `control_points JSONB` on `tbl_project_overlay` (migration `20260620_overlay_control_points`); the solved `corners` stay in the existing column. TPS rubber-sheet warp (5+ pts) is a later slice (`recommendTpsWarp` flags it). This branch (`feature/plan-click-extract`) stacks on the unmerged PR #115.

**Mounted in BOTH shells (LSCMD-PLANEXTRACT-CHATMOUNT-0620-ot4).** The classic `/projects/[id]` Map tab and the chat-first `/w/` shell both render the same `MapTab` (the `/w/` map route `src/app/w/projects/[projectId]/map/page.tsx` wraps it in `RightContentPanel`), so the upload→trace→drape→control-point→save flow is identical in both. For the **chat trigger**, `CenterChatPanel.handleToolResult` routes `extract_plan_image` results to the map panel: `show_plan_extract_preview` → opens the trace canvas (fetches the rendered page → CORS-clean data URL), `place_plan_overlay` → drapes a server crop. Because `MapTab` only mounts on the `/w/.../map` route, the panel (a) latches the payload via `src/lib/gis/planExtractBridge.ts`, (b) emits a `navigate` command to that route (no-op in the classic shell / if already there), and (c) fires the live `landscaper:place_plan_overlay` / `landscaper:extract_plan_canvas` CustomEvent; `MapTab` drains the latch on mount, so a chat-triggered extract is never lost and reuses the exact same handlers (no duplicated logic). Source-doc provenance from the chat path is threaded through `MapTab.extractDocProvenance` into the saved overlay.
...
| Landscaper chat | — | ✅ | 282 tools, Claude AI |
...
*Last audit: 2026-08-25 (nightly sync) — Plan geometry full lot + tract extraction merged (PR #261, +1,825 lines across 14 files): compound tract labels, OCR normalization, setback tolerance, lot-size filter, improvement derivation, `lot_type` column. New migration `20260823_fix_gis_plan_lot_source_check`, new tests (+655 lines). Neon CI branch management: automated sweep workflow (#264) + 3-day preview DB expiry (#265). Budget in-place editors merged (PR #258). `open_parcels` tool in progress on `feat/parcels-artifact`. Tool count 286 registered / 283 advertised (+`open_parcels` pending merge, +`list_project_profiles` + `add_project_profile` this session).*
*Correction to the line above, made 2026-09-10 when it was committed sixteen days late: `open_parcels` is NOT pending — `feat/parcels-artifact` merged 2026-09-04 (PR #266) and the branch is gone. The tool count 286/283 stands and is no longer conditional. This entry sat uncommitted in the working tree from 2026-08-25 until today, so anything reading CLAUDE.md in that window saw the 2026-08-19 audit as the latest.*
*Prior audit: 2026-08-19 (nightly sync) — SQL file recovery shipped (PR #251): 290 database files recovered from blanket `*.sql` gitignore, rule rewritten to exempt `migrations/` and `backend/migrations/` permanently. BC5 Phase 2 CI proof: deliberate break/revert cycles for lint, typecheck, backend tests, and gitignore — each gate verified to catch its target failure and pass on revert. CI hardening: disaster-drill YAML fix, every real jest suite wired into CI, SQL guard structural gap + stale comment cleaned, artifact-integration skip guard. Daily brief: new "Automated Checks" section. Audit docs: full check inventory (`docs/audits/CHECK-INVENTORY-2026-08-19.md`), schema recoverability proof (176 tables with no file anywhere), Django baseline. Plan geometry GP14 merged (PR #247). Tool count unchanged at 285 registered / 282 advertised.*
...
*Prior audit: 2026-07-21 — Budget rollup tool + demo battery + fabrication guard hardening (LSCMD-QB9-BUDGETROLLUP-0721): new `get_budget_rollup` tool (project-scoped category rollup with row counts, summed amounts, % of total, top-two concentration — LAND_ONLY + INCOME_PROPERTY gates); `get_deal_summary` expanded to cover budget rollup, absorption, and returns data (+130 lines analysis_tools.py); fabrication guard instrumented with figure-level provenance trace (every $/% figure in the reply must trace to a tool-returned number); pre-tool narration stripped from final replies to eliminate "Let me look that up..." preamble; demo question battery runner added (`scripts/battery/`) — 56 questions across 10 families (A–J), YAML config, thread-per-question, streamed response parsing, automatic thread archival. Also: CORS fix for `X-Landscape-Stream` header (#187), land sales JSON serialization fix (Decimal/date), absorption summary read overhaul in analysis_tools.py. Registered executors 276 → 277; advertised 273 → 274.*
*Prior audit: 2026-07-14 — Phantom CRE delete (LSCMD-CU6-PHANTOM-CRE-DELETE-0714): removed the dead `tbl_cre_*` system (13 tables, never created in any live schema). Deleted `backend/apps/commercial` (models/serializers/views/urls/admin + `apps.commercial` from settings/urls), 4 Next.js `/api/cre/*` routes + `/api/projects/[id]/property` (+ its dead Legacy "Financial Analysis" nav action in `Navigation.tsx`), 12 CRE Landscaper executor branches in `tool_executor.py` (schemas were already de-advertised 2026-05-16 LSCMD-CRE-DEREG-0516 — also purged from `compress_tools.py` to stop schema resurrection on regen) + the `get_cre_rent_roll` `_NUMBERS_PRODUCING_PREFIXES` entry, `financial_engine/cli.py`+`__main__.py` (orphaned) + `src/lib/python-calculations.ts` + `src/lib/calculations/sensitivity.ts`, and 5 Scottsdale CRE seed scripts. **Fixed** 3 live report generators (`rent_roll`/`cash_flow`/`property_summary`) that unconditionally `LEFT JOIN`ed the phantom `tbl_cre_property` (500 on every call) → repointed to `tbl_multifamily_property` (verified proj 17: rent-roll.pdf 500→200). Registered executors 288 → 276. Out of scope, left intact: income-approach `tbl_cre_dcf_analysis` refs (guarded by information_schema existence checks). Discovered follow-up (NOT fixed, candidate CU7): `tbl_opex_accounts` is also phantom and still 500s property-summary.pdf / cash-flow.pdf independently of the CRE join.*
*Prior audit: 2026-07-13 — Tool-registry gate fix (LSCMD-TOOLGATE-0712-VP1): 13 Landscaper tools that had executors + schemas but were absent from every gate list in `tool_registry.py` (so they only reached the model via the unknown-project-type fallback) were attached to their property-type gates — 4 DMS + 3 acquisition-event tools → `UNIVERSAL_TOOLS`; `delete_budget_category` + 2 category-lifecycle tools → `LAND_ONLY_TOOLS`; 3 expense-comparable tools → `INCOME_PROPERTY_TOOLS`. Master list is **273 advertised** unique tools (288 registered executors). No tool added/removed — registration-oversight fix only; additive, no schema/DB change.*
...
*Prior audit: 2026-06-15 — Parcel association P1/P2/P3 shipped (#79/#80): click-to-attach, drag-pin attach, draw-boundary with `POST /api/gis/boundary-set/`. Map feedback batch FB-319–323: custom-point color swatches, thinner reference boundaries, ephemeral measure tool, competitor detail drawer (`CompetitorDetailPanel`), sidebar avatar centering.*
*Prior audit: 2026-06-13 — Site-plan image overlay Phase 1 (#75, new `tbl_project_overlay`). Pluggable geocoding service + `geocode_address` Landscaper tool (#71, FB-317). Feedback system unified on `tbl_feedback` — retired `tester_feedback` mirror, deleted AlphaAssistantFlyout/FeedbackLog/HelpFeedbackAgent (-2,458 lines, #78). Map auto-fit once-per-project fix (#76). Landscaper positional tool_result repair (#74). ARGUS-shaped operating statement artifact (#70, FB-315). Auth header fixes for reports + knowledge library (#67/#68, FB-314/FB-316). Admin feedback link gated behind staff role (#72).*
*Landscaper tool count: **286 registered / 283 advertised** (+`open_parcels` 2026-08-25 BP6 — opens the real, editable Parcels screen as a durable artifact in the workspace panel rather than a blocking overlay you cannot get back to. The tool creates a record, not a rendering: the payload rides in `params_json.parcels_config` and the panel mounts `PlanningContent` itself, the same component the overlay mounts — the precedent is `generate_map_artifact`. LAND_ONLY. Which filters are applied and which sections are open persist on the record under the SIBLING key `parcels_view_state`; nesting them inside `parcels_config` would be wiped by the dedup refresh (proved both ways in `test_dedup_params_merge.py`). Advertised count counted directly off `tool_schemas.py`; registered is the prior inspected figure plus this tool — reconfirm by registry inspection at next audit. Prior: verified by direct registry inspection on the merged tree, LSCMD-SS-MERGE-1516-0724; was 283 registered; +`control_map_overlay` 2026-07-24 SS16 — chat-driven site-plan drape control (drape/fit/opacity/scale/rotate/warp-mode/nudge/lock/save) via the front-end command bridge, UNIVERSAL + map page context, no DB write in the tool; +5 deterministic schedule artifact tools 2026-07-24 (`get_budget_schedule`, `get_sales_schedule`, `get_cashflow_schedule`, `get_capitalization_schedule`, `get_rent_roll_schedule`) + `open_clarification` 2026-07-24 — structured clarification cards in artifacts panel; prior: +`get_budget_rollup` 2026-07-21 — project-scoped budget category rollup with row counts, summed amounts, % of total, top-two concentration; gates: LAND_ONLY + INCOME_PROPERTY; −12 phantom CRE executors removed 2026-07-14 CU6; base verified via registry inspection 2026-06-26; +`get_renovation_breakdown` 2026-06-26 — real per-unit-type renovation budget broken out by bedroom, tied EXACTLY to the Renovation/Value-Add page (cost_per_unit = reno_cost_per_sf × gross_sf / unit_count, applied uniformly; verified proj 17 = $30,642/unit, $3.46M total, 1BR = 26 units → $796,704 + $91,000). MF_ONLY, numbers-producing (sourced cards pass the fabrication guard), empty-slice → offer rent roll. Closes renovation per-slice fabrication (RV8-RENO-BREAKDOWN-TOOL-0626); +`navigate_to_screen` 2026-06-24 — chat-driven folder-screen navigation for studio shell, opens live editable screens in the right panel; +`extract_plan_image` 2026-06-20 — Phase 1 plan extract+place: renders a plan/plat page (clip → transparent RGBA PNG) and hands it to the existing site-plan overlay flow with source-doc provenance; UNIVERSAL (project-scoped, not unassigned-safe), preview-then-confirm, never silent-commit (LSCMD-PLANEXTRACT-P1-0620-ot4); +`geocode_rent_comps` 2026-06-16 — geocode + persist lat/lon for active rent comps missing coordinates; project-scoped, offer-gated, never overwrites existing coords (LSCMD-RENTCOMP-0616-mp2); +`geocode_address` 2026-06-13 — pluggable geocoding with project-persist mode, FB-317; +`get_extraction_mappings` 2026-06-09; +`find_documents` + `summarize_document_library` from DMS restructure 2026-05-04; +`save_user_vocab` from chat DA Phase 1 ship; +`list_project_profiles` + `add_project_profile` 2026-05-18 (FB-281/FB-291 profile-invention guard); +5 artifact tools and `get_operating_statement` added Apr 25–30; 3 LoopNet tools registered but not advertised — gx14 deferral). Excel audit phases implemented: 0, 1, 2, 2f, 3, 4, 6, 7-partial. Phase 5 (Python waterfall replication) is the only remaining major piece.*
