...
**Studio shell (new, additive) — `/studio/[projectId]` route.** A third, isolated project surface (`LSCMD-UIWRAP-LEFTNAV-0623-JB1`) that relocates the folder/sub-tab tabs into a collapsible **two-level left rail** (generated from `createFolderConfig`), with reused `CenterChatPanel` in the center and `ProjectContentRouter` output inside a reused `RightContentPanel` on the right. Top-level route (escapes the `/w/` `WrapperLayout`); composes its own `WrapperUIProvider` + project providers. Components in `src/components/studio/` (`StudioShell.tsx`, `StudioLeftNav.tsx`), styles in `src/styles/studio.css`. Does not alter classic `/projects/[id]` or `/w/`.
