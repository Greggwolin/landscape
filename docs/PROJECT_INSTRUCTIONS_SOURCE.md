...
>
> **Section tagging.** Each top-level section (`## N.0 ...`) is preceded by an HTML comment marker:
> - `<!-- TARGET: both -->` — appears in both flavors
> - `<!-- TARGET: cowork -->` — Cowork-only
> - `<!-- TARGET: claudeai -->` — Claude.ai-only
>
> The marker applies until the next top-level section header (or end of file). Within a section, finer-grained per-flavor variation is wrapped in inline blocks:
>
> ```
> <!-- BEGIN: cowork -->
> ...content for Cowork only...
> <!-- END: cowork -->
> ```
>
> The generator strips all markers and keeps only the content matching the target flavor.
