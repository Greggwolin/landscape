...
### BUG-003 Fixed: DMS Filter Shows Count But No Documents (Dec 23, 2025) ⭐ NEW
- ✅ **Root Cause** - `mv_doc_search` was a stale materialized view not being refreshed
- ✅ **Fix Applied** - Converted from materialized view to regular view for always-current data
- ✅ **Result** - Filter counts now match displayed documents (e.g., "Property Data ~20" shows 20 docs)
- 📁 Database Change: `DROP MATERIALIZED VIEW mv_doc_search; CREATE VIEW mv_doc_search AS ...`
- 🎯 Status: Complete - View now auto-updates with core_doc changes

### BUG-004 Fixed: AI Creating Unauthorized Doc Types (Dec 23, 2025) ⭐ NEW
- ✅ **Root Cause** - `DocumentClassifier` used internal types (e.g., `offering_memorandum`) instead of template-defined types
- ✅ **Fix Applied** - Added template doc_type validation and mapping in classifier
- ✅ **New Functions**:
  - `get_valid_doc_types_for_project()` - Fetches valid doc_types from project's DMS template
  - `map_to_valid_doc_type()` - Maps internal AI types to valid template options
  - `DOCTYPE_TO_TEMPLATE_MAPPING` - Fallback mapping (e.g., `offering_memorandum` → `Property Data`)
- ✅ **Data Cleanup** - Fixed 28 rogue doc_types:
  - `offering_memorandum` → `Diligence` (1 doc, project 42)
  - `project_document` → `Property Data` (27 docs, project 8)
- 📁 File Modified: `backend/apps/knowledge/services/document_classifier.py` (+100 lines)
- 🎯 Status: Complete - AI classification now constrained to template options
...
| BUG-003 | DMS filter shows count but no documents display | ✅ Fixed | DB change |
| BUG-004 | AI classifier creating unauthorized doc_types | ✅ Fixed | Pending |
...
### Schema Changes (Dec 23)
- `landscape.mv_doc_search` - Converted from materialized view to regular view (always-current document search)
