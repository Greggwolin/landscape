...
SCENARIOS = {
    ...
    's14': 'scenario_s14',
    's15': 'scenario_s15',
    's15ld': 'scenario_s15ld',
    's16': 'scenario_s16',
    's17': 'scenario_s17_budget',
}
...
def run_scenario(name: str, report: TestReport):
    ...
    if name == 's1':
        ...
        from .scenario_s7 import ScenarioS7
        agent = ScenarioS7()
        result, validation = agent.get_results()
        report.add_scenario(result, validation)
    elif name == 's8':
        ...
        from .scenario_s14 import ScenarioS14
        agent = ScenarioS14()
        result, validation = agent.get_results()
        report.add_scenario(result, validation)
    elif name == 's15':
        from .scenario_s15 import ScenarioS15
        agent = ScenarioS15()
        result, validation = agent.get_results()
        report.add_scenario(result, validation)
    elif name == 's15ld':
        from .scenario_s15ld import ScenarioS15LD
        agent = ScenarioS15LD()
        result, validation = agent.get_results()
        report.add_scenario(result, validation)
    elif name == 's16':
        from .scenario_s16 import ScenarioS16
        agent = ScenarioS16()
        result, validation = agent.get_results()
        report.add_scenario(result, validation)
    elif name == 's17':
        from .scenario_s17_budget import ScenarioS17Budget
        agent = ScenarioS17Budget()
        result, validation = agent.get_results()
        report.add_scenario(result, validation)
