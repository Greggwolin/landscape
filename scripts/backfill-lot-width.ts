 ...
 *
 * This script:
 * 1. Finds all parcels with a lot_product but lot_width = 0 or NULL
 * 2. Extracts lot_width from the product code (e.g., "50x125" → width = 50)
 * 3. For products from res_lot_product table, fetches the actual lot_w_ft value
 * 4. Updates tbl_parcel.lot_width
 */
...
async function main() {
  ...
  const resLotProducts = await sql`
