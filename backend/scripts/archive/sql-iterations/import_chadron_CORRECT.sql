-- ============================================================================
-- CHADRON RENT ROLL DATABASE IMPORT (Correct Schema)
-- ============================================================================
...
-- Insert data
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('100', 'Commercial', 1101, 3303, 3.0, 3578, 3.25, 'occupied', FALSE, 'Commercial space');
...
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('210', '2BR/2BA', 1035, 2500, 2.42, 3150, 3.04, 'occupied', FALSE, 'Balcony unit');
...
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('220', '2BR/2BA', 1035, 2200, 2.13, 3150, 3.04, 'occupied', FALSE, 'Balcony unit');
...
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('230', '3BR/2BA', 1280, 2790, 2.18, 3825, 2.99, 'occupied', FALSE, 'Balcony unit');
...
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('310', '2BR/2BA', 1035, 1700, 1.64, 3150, 3.04, 'occupied', TRUE, 'Balcony unit, Section 8');
...
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('320', '2BR/2BA', 1035, 2200, 2.13, 3150, 3.04, 'occupied', FALSE, 'Balcony unit');
...
INSERT INTO landscape.tmp_chadron_rent_roll VALUES ('330', '3BR/2BA', 1280, 3000, 2.34, 3825, 2.99, 'occupied', FALSE, 'Balcony unit');
...
SET
    ...
    market_rent_psf = (t.market_monthly_rent / t.sf)::DECIMAL(6,2),
    current_rent = t.current_monthly_rent::DECIMAL(10,2),
    current_rent_psf = CASE WHEN t.current_monthly_rent IS NOT NULL 
        THEN (t.current_monthly_rent / t.sf)::DECIMAL(6,2) 
        ELSE NULL END,
    occupancy_status = CASE WHEN t.status = 'vacant' THEN 'VACANT' ELSE 'OCCUPIED' END,
    is_section8 = t.is_section_8,
...
-- Create leases (only for occupied units)
-- First expire all existing leases
...
SELECT
    u.unit_id,
    'Current Tenant' as resident_name,
...
SELECT 'Total Units' as metric, COUNT(*) as count FROM landscape.tbl_multifamily_unit WHERE project_id = 17
UNION ALL
SELECT 'Occupied (Active Leases)' as metric, COUNT(*) FROM landscape.tbl_multifamily_lease l
    JOIN landscape.tbl_multifamily_unit u ON l.unit_id = u.unit_id
    WHERE u.project_id = 17 AND l.lease_status = 'ACTIVE'
UNION ALL
SELECT 'Vacant' as metric, COUNT(*) FROM landscape.tbl_multifamily_unit u 
...
SELECT '=== IMPORT COMPLETE - REVIEW ABOVE ===' as result;
SELECT 'To COMMIT: type COMMIT;' as next_step;
SELECT 'To ROLLBACK: type ROLLBACK;' as alternative;

-- Transaction left open for manual commit/rollback
