...
\echo 'Loading rent roll data from temp file...'

-- Load data from temp_table_inserts.sql
\i temp_table_inserts.sql

\echo ''
\echo '✅ Temporary table created and populated'

-- Verify data loaded
SELECT COUNT(*) as units_loaded FROM landscape.tmp_chadron_rent_roll;
...
SELECT
    ...
    COUNT(*) as actual,
    13 as expected,
    CASE WHEN COUNT(*) BETWEEN 5 AND 15 THEN '✅ PASS' ELSE '⚠️  REVIEW' END as status
FROM landscape.tbl_unit u
LEFT JOIN landscape.tbl_lease l ON u.unit_id = l.unit_id AND l.lease_status = 'ACTIVE'
