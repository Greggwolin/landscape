...
LANDSCAPER_TOOLS = [
    ...
    # ─── Generative Artifacts (Finding #4, Phase 1) ──────────────────────────
    {
        ...
        "description": (
            ...
            "COMPLETENESS — when an artifact represents a list of rows from a tool's "
            "response (rent roll units, comp grid entries, debt schedule lines, etc.), "
            "include EVERY row from the source data. Do NOT truncate to a sample, do "
            "NOT show only the first N rows. If get_units returned 113 units, the "
            "artifact's unit-detail table must have 113 rows. The right panel is "
            "scrollable — long lists are expected and supported. The only exception "
            "is if the source has more than 1000 rows, in which case include the "
            "first 500 plus a flag noting the truncation. "
