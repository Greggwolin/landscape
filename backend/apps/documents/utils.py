"""Utility functions for the Documents application."""
...
def detect_extract_type(filename: str, detected_doc_type: str = None) -> str:
    """
    Maps document filename and detected type to the correct extract_type
    for DMSExtractQueue.

    Returns one of: 'rent_roll', 'land_use_plan', 'entitlement',
                    'budget', 'offering_memo', 'general', or 'skip'
    """
    # Priority 1: Use detected_doc_type if provided and mappable
    ...
    # Priority 2: Filename pattern matching (case-insensitive)
    if not filename:
        return 'general'
    ...
    # Entitlement / CMP patterns
    if re.search(
        r'\bcmp\b|master\s*plan|community\s*plan|entitlement|zoning',
        name_lower,
    ):
        return 'entitlement'

    # Budget / financial patterns
    if re.search(
        r'budget|cost|pro\s*forma|proforma|t-?12|operating\s*statement',
        name_lower,
    ):
        return 'budget'

    # Offering memo patterns
    if re.search(r'\bom\b|offering\s*memo(?:randum)?', name_lower):
        return 'offering_memo'

    # Land use / plat / survey patterns
