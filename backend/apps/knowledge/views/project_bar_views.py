...
def _get_project_basics(project_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        'analysis_type': row[3],
        'analysis_perspective': row[4],
        'analysis_purpose': row[5],
        'value_add_enabled': bool(row[6]) if row[6] is not None else False,
        'project_address': row[7],
        'city': row[8],
        'state': row[9],
        'description': row[10],
        'total_units': row[11],
        'gross_sf': _decimal_to_float(row[12]),
        'acres_gross': _decimal_to_float(row[13]),
        'lot_size_acres': _decimal_to_float(row[14]),
        'proforma_noi': _decimal_to_float(row[15]),
        'current_noi': _decimal_to_float(row[16]),
        'cap_rate_proforma': _decimal_to_float(row[17]),
        'cap_rate_current': _decimal_to_float(row[18]),
        'discount_rate_pct': _decimal_to_float(row[19]),
        'net_rentable_area': _decimal_to_float(row[20]),
    }
...
def _score_project_section(project_id: int, project_basics: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fields_to_check = [
        ...
        ('analysis_perspective', project_basics.get('analysis_perspective')),
        ('analysis_purpose', project_basics.get('analysis_purpose')),
