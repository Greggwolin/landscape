...
const INDICATOR_CONFIG = [
  { id: 'population', label: 'Population', codes: ['POP_US', 'POP_STATE', 'POP_MSA', 'POP_MICRO', 'POP_COUNTY', 'ACS_POPULATION', 'ACS_COUNTY_POPULATION', 'ACS_MICRO_POPULATION', 'POP_'], format: 'number' as const },
          ...
          <div>
            ...
            >
              {analysis.summary.split('\n\n').map((para, i) => (
                <p key={i} style={{ marginBottom: i < analysis.summary.split('\n\n').length - 1 ? '0.5rem' : 0 }}>
                  {para}
                </p>
              ))}
            </div>
...
export default function LocationSubTab({ project }: LocationSubTabProps) {
  ...
  const indicatorCards = useMemo<IndicatorCard[]>(() => {
    ...
    return INDICATOR_CONFIG.map((config) => {
      ...
      for (const level of GEO_LEVEL_ORDER) {
        ...
        // Skip geo levels with no series data — don't clutter cards with dashes
      }
      return {
        id: config.id,
        label: config.label,
    ...
    async (tier: TierKey, analysis: TierAnalysis) => {
      try {
        ...
        if (!res.ok) {
          ...
          console.error(`[LocationAnalysis] Save failed for ${tier} (${res.status}):`, errBody);
          setAnalysisError((prev) => ({
            ...prev,
            [tier]: `Generated but failed to save: ${errBody.error || res.statusText}. Try regenerating.`,
          }));
        }
      } catch (saveErr) {
        console.error(`[LocationAnalysis] Save network error for ${tier}:`, saveErr);
        setAnalysisError((prev) => ({
          ...prev,
          [tier]: `Generated but failed to save: network error. Try regenerating.`,
        }));
      }
    },
    [seriesData, project.project_id],
  );

  // Update analysis content (e.g., from Landscaper edits) and persist
  const updateAnalysis = useCallback(
    async (tier: TierKey, updates: Partial<TierAnalysis>) => {
      setAnalyses((prev) => {
        const current = prev[tier];
        if (!current) return prev;
        const updated = { ...current, ...updates };
        // Persist asynchronously
        saveAnalysisToDB(tier, updated);
  ...
  const generateAnalysis = useCallback(
    ...
    [geoData, generating, project, city, state, indicatorCards, saveAnalysisToDB],
  );

  // Load saved analyses from database on mount (no dependency on seriesData)
  useEffect(() => {
    if (!project.project_id) return;

    let cancelled = false;
    const loadSaved = async () => {
      console.log(`[LocationAnalysis] Loading saved analyses for project ${project.project_id}`);
      for (const tier of ['t1', 't2', 't3'] as TierKey[]) {
        try {
          ...
          console.log(`[LocationAnalysis] Load ${tier}:`, data ? `found (v${data.version_number})` : 'null');
          if (!data?.analysis) {
            console.warn(`[LocationAnalysis] Load ${tier}: data exists but data.analysis is falsy`, { hasData: !!data, analysisType: typeof data?.analysis, analysisKeys: data?.analysis ? Object.keys(data.analysis) : 'N/A' });
            continue;
          }
          if (cancelled) {
            console.warn(`[LocationAnalysis] Load ${tier}: cancelled=true, skipping setAnalyses (component likely unmounted)`);
            continue;
          }
          // Ensure loaded analysis has correct shape (guard against double-stringified JSONB)
          let parsed = data.analysis;
          if (typeof parsed === 'string') {
            try { parsed = JSON.parse(parsed); } catch { /* use as-is */ }
          }
          console.log(`[LocationAnalysis] Load ${tier}: setting analysis state`, { tier: parsed.tier, title: parsed.title, hasSummary: !!parsed.summary, sections: parsed.sections?.length });
          setAnalyses((prev) => ({ ...prev, [tier]: parsed }));
          if (tier === 't1') t1AutoGenerated.current = true; // Prevent re-generation
        } catch (err) {
          console.error(`[LocationAnalysis] Load ${tier} error:`, err);
    ...
    loadSaved();
    return () => {
      console.warn('[LocationAnalysis] Cleanup: setting cancelled=true (component unmounting or project changed)');
      cancelled = true;
    };
  }, [project.project_id]);

  // Check staleness once both saved analyses and seriesData are available
  useEffect(() => {
    if (!project.project_id || !seriesData.length) return;
    // Only check if at least one analysis is loaded
    const hasAny = analyses.t1 || analyses.t2 || analyses.t3;
    if (!hasAny) return;
        ...
        for (const tier of ['t1', 't2', 't3'] as TierKey[]) {
          ...
          try {
            ...
            if (data?.data_snapshot && !cancelled) {
              const result = checkSnapshotStaleness(data.data_snapshot, currentSnapshot);
  ...
  }, [project.project_id, seriesData, !!analyses.t1, !!analyses.t2, !!analyses.t3]);

  // Auto-generate T1 once indicator data loads (only if not loaded from DB)
  // T1 no longer auto-generates on page load — all tiers require manual trigger
  // Analyses are persisted to DB and loaded on mount, so re-generation is only
  // needed when the user explicitly clicks "Generate" or "Update".
