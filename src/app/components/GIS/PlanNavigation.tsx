  ...
  const renderBreadcrumbs = (parcel: PlanParcel) => {
    ...
    return breadcrumbs.map((item, index) => (
      <React.Fragment key={item}>
        <span className={index === breadcrumbs.length - 1 ? 'text-blue-400 font-medium' : 'text-gray-400'}>
          {item}
        </span>
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2">
          <button
            ...
            className="bg-gray-800 border border-gray-700 rounded-lg p-2 text-white hover:bg-gray-700 shadow-lg"
          ...
          <select
            ...
            className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white hover:bg-gray-700 shadow-lg"
          >
        ...
        {/* Stats Bar */}
        {stats && (
          <div className="absolute bottom-4 right-4 bg-gray-800 border border-gray-700 rounded-lg p-3 shadow-lg">
            ...
            {selectedParcel ? (
              <div className="p-4 space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-gray-400 uppercase tracking-wide">Parcel Code</label>
                    <div className="text-white font-mono">{selectedParcel.parcel_code}</div>
                  </div>

                  <div>
                    <label className="text-xs text-gray-400 uppercase tracking-wide">Land Use</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-gray-400 uppercase tracking-wide">Acres</label>
                      <div className="text-white">{selectedParcel.acres_gross.toFixed(2)}</div>
                    </div>

                    {selectedParcel.units_total && (
                      <div>
                        <label className="text-xs text-gray-400 uppercase tracking-wide">Units</label>
                        <div className="text-white">{selectedParcel.units_total}</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="border-t border-gray-700 pt-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs text-gray-400">Adjust Confidence:</label>
                ...
                <div className="border-t border-gray-700 pt-4">
                  ...
                  <div className="text-sm text-white bg-gray-700 rounded px-3 py-2 font-mono">
                ...
                <div className="border-t border-gray-700 pt-4">
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-mono">{selectedParcel.parcel_id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Version:</span>
                      <span className="text-white">{selectedParcel.version}</span>
                    </div>
                    {selectedParcel.area_no && (
                      <div className="flex justify-between">
                        <span className="text-gray-400">Area:</span>
                        <span className="text-white">{selectedParcel.area_no}</span>
                      </div>
                    )}
                    {selectedParcel.phase_no && (
                      <div className="flex justify-between">
                        <span className="text-gray-400">Phase:</span>
                        <span className="text-white">{selectedParcel.phase_no}</span>
                      </div>
                    )}
                  </div>
                </div>
