  ...
  try {
    const comparables = await sql<any[]>`
      SELECT
        ...
        annual_absorption,
        date_reported
      FROM landscape.market_data_absorption_rates
      WHERE project_id = ${id}
    ...
    console.error('Failed to fetch absorption rates:', error);
    return NextResponse.json(
      { error: 'Failed to fetch absorption rates', details: message },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const result = await sql`
      INSERT INTO landscape.market_data_absorption_rates (
        project_id,
        project_name,
        location,
        product_type,
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error('Failed to create absorption rate:', error);
    return NextResponse.json(
      { error: 'Failed to create absorption rate', details: message },
      { status: 500 }
    );
  }
}
