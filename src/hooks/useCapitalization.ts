...
export function useLoans(projectId: string) {
  return useQuery({
    ...
    queryFn: () => authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/`).then((r) => r.json()),
    enabled: !!projectId,
  });
}
...
export function useLoanDraws(projectId: string, loanId: number) {
  return useQuery({
    ...
    queryFn: () =>
      authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/${loanId}/draws/`).then((r) => r.json()),
...
export function useLoanBalanceSummary(projectId: string, loanId: number) {
  return useQuery({
    ...
    queryFn: () =>
      authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/${loanId}/balance-summary/`).then((r) => r.json()),
    enabled: !!projectId && !!loanId,
  });
}
...
export function useCreateLoan(projectId: string) {
  ...
  return useMutation({
    mutationFn: (data: Partial<Loan>) =>
      authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/`, {
        method: 'POST',
...
export function useUpdateLoan(projectId: string) {
  ...
  return useMutation({
    mutationFn: ({ loanId, data }: { loanId: number; data: Partial<Loan> }) =>
      authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/${loanId}/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      }).then((r) => {
...
export function useLoanSchedule(projectId: string, loanId: string | number | null) {
  return useQuery({
    ...
    queryFn: () =>
      authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/${loanId}/debt-schedule/`).then((r) =>
        r.json()
      ),
    enabled: !!projectId && !!loanId,
  });
...
export function useLoanBudgetSummary(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/${loanId}/budget-summary/`);
      if (!response.ok) {
...
export function useCalculateInterestReserve(projectId: string, loanId: number | null) {
  ...
  return useMutation({
    mutationFn: async () => {
      const response = await authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/loans/${loanId}/interest-reserve/calculate/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({}),
        });
...
export function useLeveragedCashFlow(
  ...
  return useQuery({
    ...
    queryFn: () =>
      authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/cash-flow/calculate/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
export function useIncomeApproachMonthlyDCF(projectId: string, enabled: boolean = true) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await authFetch(`${DJANGO_API_URL}/api/valuation/income-approach-data/${projectId}/dcf/monthly/`);
      if (!response.ok) {
...
export function useAcquisitionPriceSummary(projectId: string, enabled: boolean = true) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await authFetch(`${DJANGO_API_URL}/api/projects/${projectId}/acquisition/price-summary/`);
      if (!response.ok) {
