...
export default function WaterfallConfigForm({
  projectId,
  onSaved,
  waterfallType: controlledWaterfallType,
  onWaterfallTypeChange,
}: WaterfallConfigFormProps) {
    ...
    async function loadData() {
      try {
        ...
        if (equityRes.ok) {
          ...
          if (!cancelled && typeof equity === 'number' && Number.isFinite(equity) && equity > 0) {
            setTotalEquityRequired(equity);
            // peakEquity auto-loaded from cash flow summary
            equityFromCashFlow = true;
          }
        }
        ...
        if (waterfallRes.ok && !cancelled) {
          ...
          // If peakEquity wasn't available (non-LAND projects), try partner capital
          if (!cancelled && !equityFromCashFlow) {
            const partnerTotal = partners.reduce(
              (sum: number, p: any) => sum + (Number(p.capitalCommitted) || Number(p.commitmentAmount) || 0),
              0
            );
            if (partnerTotal > 0) {
              setTotalEquityRequired(partnerTotal);
              // peakEquity auto-loaded from cash flow summary
            }
          }
        }
  ...
  const handleSave = useCallback(async () => {
    ...
    // Note: effectiveTotalEquity may be 0 for non-LAND projects where peakEquity
    // is not auto-derived. The waterfall calc engine fetches its own cash flows
    // independently, so saving tier config is always valid.
