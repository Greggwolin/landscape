      ...
      <table className="ops-table ops-grid">
          ...
          <tbody>
            ...
            <tr className="ops-parent-row">
              <td className="font-semibold" style={{ whiteSpace: 'nowrap' }}>Rental Income</td>
              <td className="text-right font-semibold" style={{ whiteSpace: 'nowrap' }}>Units</td>
              <td className="text-right font-semibold" style={{ whiteSpace: 'nowrap' }}>Current</td>
              ...
              <td className="text-right font-semibold" style={{ whiteSpace: 'nowrap', fontSize: '0.8125rem' }}>Loss to Lease</td>
              {valueAddEnabled && (
                <>
            ...
            {rentalRows.map((row) => {
              ...
              return (
                <tr key={row.line_item_key} className="ops-child-row" style={{ color: 'var(--cui-body-color)' }}>
                  ...
                  <td className="text-right py-1 font-semibold">{currentTotal > 0 ? formatCurrency(currentTotal) : '—'}</td>
                  <td className="text-right py-1">{perSF ? formatPerSF(perSF) : '—'}</td>
                  <td
                    className="text-right py-1 ops-loss-to-lease"
                  ...
                  {valueAddEnabled && (
                    <>
                      ...
                      <td className="text-right py-1 font-semibold post-reno">{postRenoTotal > 0 ? formatCurrency(postRenoTotal) : '—'}</td>
                    </>
                  )}
                </tr>
              );
            ...
            {otherIncomeRows.length > 0 && (
              <>
                <tr className="ops-parent-row">
                  <td className="font-semibold" style={{ whiteSpace: 'nowrap' }}>Other Income</td>
                  <td className="text-right font-semibold">—</td>
                  <td className="text-right font-semibold">—</td>
                  <td className="text-right font-bold">{formatCurrency(otherIncomeTotal)}</td>
                  <td className="text-right font-semibold">—</td>
                  <td className="text-right font-semibold">—</td>
                  {valueAddEnabled && (
                    <>
                      <td className="text-right font-semibold post-reno">—</td>
                      <td className="text-right font-bold post-reno">—</td>
                    </>
                  )}
                </tr>
                ...
                {otherIncomeRows.map((row) => {
                  ...
                  return (
                    <tr key={row.line_item_key} className="ops-child-row" style={{ color: 'var(--cui-body-color)' }}>
                      ...
                      <td className="text-right py-1 font-semibold">{(row.as_is.total || 0) > 0 ? formatCurrency(row.as_is.total || 0) : '—'}</td>
                      <td className="text-right py-1">—</td>
                      <td className="text-right py-1">—</td>
                      {valueAddEnabled && (
                        <>
            ...
            {vacancyRows.length > 0 && (
              <>
                <tr className="ops-parent-row">
                  <td className="font-semibold" style={{ whiteSpace: 'nowrap' }}>Less: Vacancy & Deductions</td>
                  <td></td>
                  <td></td>
                  <td></td>
                  <td></td>
                    ...
                    <tr key={row.line_item_key} className="ops-child-row" style={{ color: 'var(--cui-body-color)' }}>
                      <td className="py-1" style={{ whiteSpace: 'nowrap' }}>
                        <span className="inline-flex items-center gap-1">
                          {row.label}
                          {isReadOnly && (
                            <span className="inline-flex items-center gap-1 text-xs ml-1" style={{ color: 'var(--cui-secondary-color)' }}>
                      ...
                      <td className="text-right py-1 ops-vacancy-value font-semibold">{amount !== 0 ? `(${formatCurrency(Math.abs(amount))})` : '—'}</td>
                      ...
                      {valueAddEnabled && (
                        <>
                          ...
                          <td className="text-right py-1 ops-vacancy-value font-semibold post-reno">{postRenoTotal !== 0 ? `(${formatCurrency(Math.abs(postRenoTotal))})` : '—'}</td>
                        </>
                      )}
                    </tr>
                  );
            ...
            >
              <td className="py-3 font-bold" style={{ color: 'var(--cui-body-color)' }}>
                Effective Gross Income
              </td>
              <td className="text-right py-3 font-semibold" style={{ color: 'var(--cui-body-color)' }}>
                {rentalTotals.count}
              </td>
              <td className="text-right py-3 font-semibold" style={{ color: 'var(--cui-body-color)' }}>
                —
              </td>
              <td className="text-right py-3 font-bold" style={{ color: 'var(--cui-success)' }}>
              ...
              <td
                className="text-right py-3 ops-loss-to-lease"
              ...
              {valueAddEnabled && (
                <>
                  <td className="text-right py-3 font-semibold post-reno" style={{ color: 'var(--cui-body-color)' }}>
                    —
                  </td>
                  <td className="text-right py-3 font-bold post-reno" style={{ color: 'var(--cui-success)' }}>
                    —
                  </td>
                </>
              )}
