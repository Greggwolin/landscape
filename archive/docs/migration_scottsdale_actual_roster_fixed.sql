...
DELETE FROM landscape.tbl_cre_base_rent WHERE lease_id IN (SELECT lease_id FROM landscape.tbl_cre_lease WHERE cre_property_id = 1);
DELETE FROM landscape.tbl_cre_rent_escalation WHERE lease_id IN (SELECT lease_id FROM landscape.tbl_cre_lease WHERE cre_property_id = 1);
DELETE FROM landscape.tbl_cre_expense_recovery WHERE lease_id IN (SELECT lease_id FROM landscape.tbl_cre_lease WHERE cre_property_id = 1);
DELETE FROM landscape.tbl_cre_tenant_improvement WHERE lease_id IN (SELECT lease_id FROM landscape.tbl_cre_lease WHERE cre_property_id = 1);
DELETE FROM landscape.tbl_cre_leasing_commission WHERE lease_id IN (SELECT lease_id FROM landscape.tbl_cre_lease WHERE cre_property_id = 1);
DELETE FROM landscape.tbl_cre_lease WHERE cre_property_id = 1;
DELETE FROM landscape.tbl_cre_tenant;
DELETE FROM landscape.tbl_cre_space WHERE cre_property_id = 1;
...
INSERT INTO landscape.tbl_cre_space (cre_property_id, space_number, usable_sf, rentable_sf, space_type, space_status) VALUES
  (1, 'PAD2', 133120, 133120, 'Power Center Anchor', 'Leased'),      -- Living Spaces
  (1, 'MAJ4', 34922, 34922, 'Major Anchor', 'Leased'),                -- Painted Tree Marketplace
  (1, 'MAJ7', 34565, 34565, 'Major Anchor', 'Leased'),                -- Nordstrom Rack
  (1, 'MAJ1', 25200, 25200, 'Major Anchor', 'Leased'),                -- Saks Off 5th
  (1, 'MAJ6', 23925, 23925, 'Major Anchor', 'Leased'),                -- Michaels
  (1, 'MAJ10', 23656, 23656, 'Entertainment Anchor', 'Leased'),       -- Putting World
  (1, 'MAJ9', 19444, 19444, 'Major Anchor', 'Leased'),                -- PetSmart
  (1, 'MAJ3', 18390, 18390, 'Major Anchor', 'Leased'),                -- Cost Plus
  (1, 'MAJ5', 13000, 13000, 'Major Anchor', 'Leased');                -- Ulta Salon

-- LARGE IN-LINE / JUNIOR ANCHORS (10,000 - 13,000 SF)

INSERT INTO landscape.tbl_cre_space (cre_property_id, space_number, usable_sf, rentable_sf, space_type, space_status) VALUES
  (1, '13', 12604, 12604, 'Restaurant - Full Service', 'Leased'),     -- Maggiano's / Corner Bakery
  (1, '7', 12500, 12500, 'Restaurant - Full Service', 'Leased'),      -- Cooper's Hawk Winery
  (1, 'MAJ8A', 11008, 11008, 'Major In-Line', 'Leased'),              -- Old Navy
  (1, 'MAJ2', 10630, 10630, 'Major In-Line', 'Leased'),               -- Five Below
  (1, 'E115', 10334, 10334, 'In-Line Retail', 'Leased'),              -- Tilly's
  (1, '12', 10000, 10000, 'Grocery - Specialty', 'Leased');           -- Trader Joe's

-- MEDIUM IN-LINE (6,000 - 10,000 SF)

INSERT INTO landscape.tbl_cre_space (cre_property_id, space_number, usable_sf, rentable_sf, space_type, space_status) VALUES
  (1, '3', 8695, 8695, 'Restaurant - Full Service', 'Leased'),        -- Benihana
  (1, '14', 8650, 8650, 'Restaurant - Full Service', 'Leased'),       -- The Capital Grille
  (1, 'PAD8', 7350, 7350, 'Pad Site - Restaurant', 'Leased'),         -- Buffalo Wild Wings
  (1, '9', 7150, 7150, 'In-Line Retail', 'Leased'),                   -- Robbins Brothers
  (1, '5A2/5', 6105, 6105, 'In-Line Retail', 'Leased'),               -- Men's Wearhouse
  (1, 'MAJ8B', 5980, 5980, 'In-Line Retail', 'Leased');               -- Skechers USA

-- STANDARD IN-LINE (3,000 - 5,000 SF)

INSERT INTO landscape.tbl_cre_space (cre_property_id, space_number, usable_sf, rentable_sf, space_type, space_status) VALUES
  (1, '5B2', 5000, 5000, 'In-Line Retail', 'Available'),              -- Vacant
  (1, 'F107', 4784, 4784, 'In-Line Retail', 'Available'),             -- Vacant
  (1, 'C4109', 4504, 4504, 'In-Line Retail', 'Leased'),               -- Jos. A. Bank
  (1, 'F115', 4500, 4500, 'In-Line Retail', 'Leased'),                -- Carter's
  (1, '4', 4447, 4447, 'Bank Branch', 'Leased'),                      -- Bank of America
  (1, 'D109', 4340, 4340, 'Fitness', 'Leased'),                       -- Anytime Fitness
  (1, '16A', 4334, 4334, 'Telecommunications', 'Leased'),             -- Verizon
  (1, '11', 4186, 4186, 'Restaurant - Casual', 'Leased'),             -- Someburros
  (1, '5A1', 4007, 4007, 'Restaurant - Bakery/Cafe', 'Leased'),       -- Paris Baguette
  (1, '10', 4000, 4000, 'Restaurant - QSR', 'Leased'),                -- In-N-Out Burger
  (1, 'F111', 4000, 4000, 'In-Line Retail', 'Leased'),                -- World of Rugs
  (1, 'E107', 3836, 3836, 'Restaurant - Casual', 'Leased'),           -- First Watch
  (1, 'D115', 3558, 3558, 'Personal Services', 'Leased'),             -- Ideal Image of Arizona
  (1, '101', 3500, 3500, 'Restaurant - Casual', 'Leased'),            -- Picazzo's Healthy Italian
  (1, 'A8', 3000, 3000, 'In-Line Retail', 'Available');               -- Vacant

-- SMALL IN-LINE (< 3,000 SF)

INSERT INTO landscape.tbl_cre_space (cre_property_id, space_number, usable_sf, rentable_sf, space_type, space_status) VALUES
  (1, '17A', 3000, 3000, 'Restaurant - Fast Casual', 'Leased'),       -- Blaze Pizza
  (1, '15A', 2869, 2869, 'Restaurant - Fast Casual', 'Leased'),       -- The Original Chop Shop Co.
  (1, '16D', 2800, 2800, 'Restaurant - Fast Casual', 'Leased'),       -- Modern Market
  (1, '17B', 2685, 2685, 'Dental Office', 'Leased'),                  -- Peachtree Dental
  (1, '104', 2640, 2640, 'Music School', 'Leased');                   -- Bach to Rock
