...
export async function PUT(
  ...
  try {
    ...
    if ('lifecycle_stage' in updates) {
      await sql`UPDATE landscape.core_fin_fact_budget SET lifecycle_stage = ${updates.lifecycle_stage} WHERE fact_id = ${factId}`;
      hasUpdates = true;
    }
    ...
    // Fetch and return the updated record with category names from either system
