...
export async function GET(request: NextRequest) {
  try {
    ...
    if (project_id) {
      ...
      if (activity) {
        ...
        const result = await sql`
          ...
          INNER JOIN landscape.core_category_activitys cls
    ...
    if (template_name && project_type_code) {
      // Note: core_fin_category doesn't have template_name or project_type_code
      // This query path is likely unused - return empty for now
      const result = await sql`
        SELECT
          category_id,
          parent_id,
          code,
          COALESCE(detail, event, class, 'Uncategorized') as name,
          kind,
          class,
          event,
          scope,
        ...
        WHERE is_active = ${!include_inactive}
        ORDER BY
          CASE
            WHEN kind = 'Use' THEN 1
            WHEN kind = 'Source' THEN 2
            ELSE 3
          END,
          class NULLS LAST,
          event NULLS LAST
