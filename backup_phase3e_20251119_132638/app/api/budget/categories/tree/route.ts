...
export async function GET(request: NextRequest) {
  try {
    ...
    if (project_id) {
      ...
      // Fetch unit cost categories with hierarchical level calculation
      // Using core_unit_cost_category (supports lifecycle stages)
      const result = await sql`
        SELECT
          ...
          CASE
            ...
            WHEN EXISTS (SELECT 1 FROM landscape.core_unit_cost_category p WHERE p.category_id = c.parent_id AND p.parent_id IS NULL) THEN 2
            WHEN EXISTS (SELECT 1 FROM landscape.core_unit_cost_category p2
                         JOIN landscape.core_unit_cost_category p1 ON p2.parent_id = p1.category_id
                         WHERE p2.category_id = c.parent_id AND p1.parent_id IS NULL) THEN 3
            ELSE 4
          END as level,
          NULL::text as code,
          ...
          NULL::text as project_type_code,
          sort_order,
          NULL::text as icon,
          NULL::text as color,
          is_active
      ...
      // Get template categories - using core_unit_cost_category
      ...
      // Get all templates for project type - using core_unit_cost_category
      const result = await sql`
        SELECT
          category_id,
          parent_id,
      ...
      // No filter - return all active categories from core_unit_cost_category
      const result = await sql`
        SELECT
          category_id,
          parent_id,
