...
/**
 * DELETE /api/sale-benchmarks/:benchmarkId
 * Delete a sale benchmark from tbl_sale_benchmarks
 */
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error('[DELETE sale-benchmark] Django error:', response.status, errorData);
      return NextResponse.json(
        { error: 'Failed to delete benchmark', details: errorData },
        { status: response.status }
      );
    }

    console.log('[DELETE sale-benchmark] Success for ID:', benchmarkId);
    return NextResponse.json({ success: true, message: 'Benchmark deleted successfully' });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error('Delete benchmark proxy error:', error);
    return NextResponse.json(
      { error: 'Failed to delete benchmark', details: message },
      { status: 500 }
    );
  }
}
