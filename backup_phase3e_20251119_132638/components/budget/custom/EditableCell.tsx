...
function TanStackEditableCell({ getValue, row, column }: TanStackProps) {
  ...
  useEffect(() => {
    if (editing && inputType === 'category-select' && meta.projectId) {
      const fetchCategories = async () => {
        try {
          // Get lifecycle_stage from the current row to filter categories
          const lifecycleStage = row?.original?.lifecycle_stage;

          // Build URL with optional lifecycle_stage filter
  ...
  }, [editing, inputType, meta.projectId, row?.original?.lifecycle_stage]);
