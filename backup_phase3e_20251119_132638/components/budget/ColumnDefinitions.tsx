...
export function getColumnsByMode(
  ...
  const stageColumn: ColumnDef<BudgetItem> = {
    ...
    minSize: 120,
    maxSize: 180,
    meta: createMeta('lifecycle_stage', 'select', [
  ...
  const phaseColumn = napkinColumns.find(col => 'accessorKey' in col && col.accessorKey === 'container_id');

  // Get napkin columns without phase for reuse in standard mode
  const napkinWithoutPhase = napkinColumns.filter(col =>
    'accessorKey' in col && col.accessorKey !== 'container_id'
  );
